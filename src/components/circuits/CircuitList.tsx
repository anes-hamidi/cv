...
export function CircuitList() {
  const { circuits, addOrUpdateCircuit, deleteCircuit, customers } = usePOS();
    ...
    <TooltipProvider>
        ...
        <div className="rounded-lg border">
            ...
            <TableBody>
                {circuits.length === 0 ? (
                    <TableRow>
                        <TableCell colSpan={4} className="h-24 text-center">No circuits found.</TableCell>
                    </TableRow>
                ) : (
                    circuits.map((circuit) => {
                        const assignedCustomers = customers.filter(c => c.circuitId === circuit.id);
                        return (
                                         ...
                                         <Tooltip>
                                            ...
                                            <TooltipContent>
                                                <ul className="list-disc pl-4">
                                                    {assignedCustomers.map(c => <li key={c.id}>{c.name}</li>)}
                                                </ul>
                                            </TooltipContent>
                                        </Tooltip>
                                    ) : (
